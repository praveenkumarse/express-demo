...
module.exports = {
    get_home_info: function (req, res, next) {
        ...
        userInfo.find({ 'info.user': user.email }).select("-info.user").exec(function (err, data) {
            if (err) return err;
            if (data[0]) {
                res.json({ info: data[0].info });
            } else {
    ...
    post_home_info: function (req, res, next) {
        ...
        userInfo.update({ }, { upsert: true, new: true, setDefaultsOnInsert: true }, function (error, result) {
             console.log(error)
            if (error) return error;
            console.log(result)
            result.info.push(data)
